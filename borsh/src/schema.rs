...
#[cfg(feature = "std")]
mod id_addr_std_copy {

    #[derive(crate::BorshSchema)]
    #[borsh(crate = "crate")]
    pub struct Ipv4Addr {
        octets: [u8; 4],
    }
...
{
    fn add_definitions_recursively(definitions: &mut BTreeMap<Declaration, Definition>) {
        <id_addr_std_copy::Ipv4Addr>::add_definitions_recursively(definitions);
    }

    fn declaration() -> Declaration {
        id_addr_std_copy::Ipv4Addr::declaration()
    }
}
...
{
    fn add_definitions_recursively(definitions: &mut BTreeMap<Declaration, Definition>) {
        <id_addr_std_copy::Ipv6Addr>::add_definitions_recursively(definitions);
    }

    fn declaration() -> Declaration {
        id_addr_std_copy::Ipv6Addr::declaration()
    }
}
...
{
    fn add_definitions_recursively(definitions: &mut BTreeMap<Declaration, Definition>) {
        <id_addr_std_copy::IpAddr>::add_definitions_recursively(definitions);
    }

    fn declaration() -> Declaration {
        id_addr_std_copy::IpAddr::declaration()
    }
}
